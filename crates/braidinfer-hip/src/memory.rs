...
impl<T> MappedHostBuffer<T> {
    ...
    ///
    /// Different kernel-driver pin path than `hipHostMalloc`:
    ///   - User-space mmap → kernel allocates from hugetlbfs (2MB pages)
    ///   - hipHostRegister → amdgpu pins the existing user pages into IOMMU/GART
    ///   - hipHostMalloc → amdgpu allocates from its own pool + pins
    /// If the §11.4 wedge mechanism is an IOMMU/GART page-table state issue
    /// specific to `hipHostMalloc`'s pin path, this probe should sidestep it.
    /// If the wedge persists, the mechanism is not in the alloc path.
    ///
    /// On failure to allocate hugepages (system lacks them), falls back to
    /// anonymous mmap so the test still runs at 4KB page granularity through
    /// the hipHostRegister path — still a different code path than
    /// hipHostMalloc.
    ///
    /// Owns the mmap region; Drop calls hipHostUnregister + munmap.
    pub fn alloc_hostregister_hugetlb(len: usize) -> HipResult<Self> {
        let size = len * std::mem::size_of::<T>();
        // Round up to 2MB (hugepage size).
        const HUGEPAGE: usize = 2 * 1024 * 1024;
        let mapped_size = (size + HUGEPAGE - 1) & !(HUGEPAGE - 1);

        let mut host_ptr = unsafe {
            libc::mmap(
                std::ptr::null_mut(),
                mapped_size,
        ...
        let used_hugepage = !host_ptr.is_null() && host_ptr != libc::MAP_FAILED;
        if !used_hugepage {
            // Fallback to anonymous 4KB pages.
            host_ptr = unsafe {
                libc::mmap(
                    std::ptr::null_mut(),
                    mapped_size,
            ...
            if host_ptr == libc::MAP_FAILED {
                return Err(error::HipError(1)); // hipErrorInvalidValue
            }
        }
        eprintln!(
            "[pky.2-iommu-probe] alloc_hostregister_hugetlb: hugepage={} size={} mapped={}",
            used_hugepage, size, mapped_size
        );

        // Zero (touches all pages — forces TLB fault + IOMMU table population).
        unsafe { ptr::write_bytes(host_ptr.cast::<u8>(), 0, mapped_size) };

        // Register with HIP runtime.
        // hipHostRegisterMapped = 0x2, hipHostRegisterPortable = 0x1
        const REGISTER_FLAGS: u32 = 0x2; // Mapped only (matches alloc())
        let rc = unsafe { ffi::hipHostRegister(host_ptr, mapped_size, REGISTER_FLAGS) };
        if rc != 0 {
            unsafe { libc::munmap(host_ptr, mapped_size) };
            return Err(error::HipError(rc));
        }

        let mut device_ptr: *mut std::ffi::c_void = ptr::null_mut();
        let rc = unsafe { ffi::hipHostGetDevicePointer(&mut device_ptr, host_ptr, 0) };
        if rc != 0 {
            unsafe {
                ffi::hipHostUnregister(host_ptr);
                libc::munmap(host_ptr, mapped_size);
            }
            return Err(error::HipError(rc));
        }
