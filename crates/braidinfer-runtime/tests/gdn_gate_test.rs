...
fn test_gdn_gate_decay_in_unit_interval() {
    ...
    let dt_bias: Vec<f32> = (0..num_heads).map(|i| (i as f32 * 0.03) - 0.5).collect();

    let expected: Vec<f32> = (0..num_heads)
        .map(|h| {
            let sp = softplus(a[h] + dt_bias[h]);
            (-a_log[h].exp() * sp).exp()
        })
        .collect();
    ...
    let mut d_dt = DeviceBuffer::<f32>::alloc(device, num_heads).expect("alloc dt_bias");
    ...
    assert!(max_err < 1e-5, "gdn_gate max error {max_err} exceeds tolerance");
