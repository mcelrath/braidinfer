...
fn test_ffn_fused_matches_unfused() {
    ...
    let mut d_wgate = DeviceBuffer::<f32>::alloc(device, INTERMEDIATE_SIZE * HIDDEN_SIZE).expect("alloc w_gate");
    let mut d_wup = DeviceBuffer::<f32>::alloc(device, INTERMEDIATE_SIZE * HIDDEN_SIZE).expect("alloc w_up");
    let mut d_wdown = DeviceBuffer::<f32>::alloc(device, HIDDEN_SIZE * INTERMEDIATE_SIZE).expect("alloc w_down");
    let mut d_scratch = DeviceBuffer::<f32>::alloc(device, INTERMEDIATE_SIZE).expect("alloc scratch");
...
fn test_ffn_fused_matches_unfused_kernels() {
    ...
    let mut d_gate_out = DeviceBuffer::<f32>::alloc(device, INTERMEDIATE_SIZE).expect("alloc gate_out");
    ...
    lp_kernel
        ...
        .expect("gate proj");
    lp_kernel
        .forward(&mut d_up_out, &d_wup, &d_normed, INTERMEDIATE_SIZE as u32, HIDDEN_SIZE as u32, &stream)
        .expect("up proj");
    silu_kernel
        .forward(&mut d_act, &d_gate_out, &d_up_out, INTERMEDIATE_SIZE as u32, &stream)
        .expect("silu_mul");
    lp_kernel
        .forward(&mut d_down_out, &d_wdown, &d_act, HIDDEN_SIZE as u32, INTERMEDIATE_SIZE as u32, &stream)
        .expect("down proj");
    res_kernel
        .forward(&mut d_ref_output, &d_down_out, &d_input, HIDDEN_SIZE as u32, &stream)
        .expect("residual_add");

    stream.synchronize().expect("sync unfused");

    let mut unfused_result = vec![0.0f32; HIDDEN_SIZE];
    d_ref_output.copy_to_host(&mut unfused_result).expect("copy unfused output");

    // --- Fused path ---
    let mut d_scratch = DeviceBuffer::<f32>::alloc(device, INTERMEDIATE_SIZE).expect("alloc scratch");
    let mut d_fused_output = DeviceBuffer::<f32>::alloc(device, HIDDEN_SIZE).expect("alloc fused output");
    ...
    d_fused_output.copy_to_host(&mut fused_result).expect("copy fused output");
