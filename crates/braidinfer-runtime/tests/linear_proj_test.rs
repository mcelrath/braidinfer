...
fn test_linear_proj_matches_reference() {
    ...
    let mut d_weight = DeviceBuffer::<f32>::alloc(device, out_dim * in_dim).expect("alloc weight");
    ...
    kernel
        .forward(&mut d_output, &d_weight, &d_input, out_dim as u32, in_dim as u32, &stream)
