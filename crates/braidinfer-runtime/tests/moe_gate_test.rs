...
fn build_instruction(opcode: u32, grid_x: u32, args: &[(usize, u64)]) -> [u64; 16] {
    ...
    for &(idx, val) in args { inst[idx] = val; }
    inst
}
...
fn test_moe_gate_softmax() {
    ...
    for i in 0..hidden_size { hidden[i] = (i as f32) * 0.1; }

    // Gate weights: [num_experts, hidden_size] as bf16
    let mut gate_data = vec![0u16; num_experts * hidden_size];
    for e in 0..num_experts {
        for h in 0..hidden_size {
            let val = if e == 2 { 0.5 } else if e == 5 { 0.3 } else if e == 7 { 0.2 } else { 0.0 };
    ...
    let mut gate_buf = DeviceBuffer::<u16>::alloc(device, num_experts * hidden_size).expect("alloc");
    ...
    let proj_inst = build_instruction(OP_LINEAR_PROJ, num_experts as u32, &[
    ...
    let gate_inst = build_instruction(OP_MOE_GATE, 1, &[
    ...
    func.launch_cooperative((384, 1, 1), (256, 1, 1), 256 * 4 * 2, &stream, &mut args).expect("launch");
    stream.synchronize().expect("sync");

    let mut ids = vec![0i32; k];
    let mut weights = vec![0.0f32; k];
    expert_ids_buf.copy_to_host(&mut ids).expect("read ids");
    expert_weights_buf.copy_to_host(&mut weights).expect("read weights");
    ...
    assert!(weights[0] > weights[1], "top expert should have highest weight");
