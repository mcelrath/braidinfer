...
fn decode_one(model_dir: &str, label: &str) {
    ...
    let mut model = match Model::load(p, device) {
        Ok(model) => model,
        Err(err) => {
            eprintln!("SKIP {label}: load failed: {err:?}");
            return;
        }
    };
    let cfg = model.config();
    println!(
        "{label}: hidden={}, layers={}, nh={}, nvh={}, tie_embed={}",
        cfg.hidden_size,
        cfg.num_layers,
    ...
    let logits = match model.decode_step(9707, 0) {
        Ok(logits) => logits,
        Err(err) => {
            eprintln!("SKIP {label}: decode failed: {err:?}");
            return;
        }
    };
    let (idx, val) = logits
    ...
    println!(
        "  argmax={idx}, logit={val:.4}, nonzero={nonzero}/{}",
        logits.len()
    );
    ...
    if nans > 0 || infs > 0 {
        println!("  WARNING: {nans} NaN, {infs} Inf");
    }
}

#[test]
#[ignore = "manual debug probe against large local models"]
...
fn test_9b_multi_step() {
    ...
    let mut model = match Model::load(p, device) {
        Ok(model) => model,
        Err(err) => {
            eprintln!("SKIP 9B multi-step: load failed: {err:?}");
            return;
        }
    };
    let mut token = 9707u32;
    for step in 0..5 {
        let logits = match model.decode_step(token, step) {
            Ok(logits) => logits,
            Err(err) => {
                eprintln!("SKIP 9B multi-step: step {step} failed: {err:?}");
                return;
            }
        };
        let (idx, val) = logits
