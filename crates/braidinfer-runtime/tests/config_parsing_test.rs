use braidinfer_runtime::model::{ModelConfig, LayerType, FfnType};
use std::path::Path;
...
fn test_parse_qwen35_0_8b() {
    ...
    let gdn_count = cfg.layers.iter().filter(|l| l.layer_type == LayerType::Gdn).count();
    let attn_count = cfg.layers.iter().filter(|l| l.layer_type == LayerType::Attention).count();
...
fn test_parse_qwen35_122b() {
    let Some(dir) = find_snapshot("Qwen--Qwen3.5-122B-A10B") else { return; };
    let cfg = parse_config(&dir).expect("config not found");
    assert_eq!(cfg.num_layers, 48);
    let gdn = cfg.layers.iter().filter(|l| l.layer_type == LayerType::Gdn).count();
    ...
    println!("Qwen3.5-122B: {gdn} GDN + {attn} Attn, MoE {}/{}", cfg.num_active_experts, cfg.num_experts);
}

#[test]
fn test_parse_nemotron_cascade_30b() {
    let Some(dir) = find_snapshot("nvidia--Nemotron-Cascade-2-30B-A3B") else { return; };
    let cfg = parse_config(&dir).expect("config not found");
...
fn test_parse_nemotron_120b() {
    let Some(dir) = find_snapshot("nvidia--NVIDIA-Nemotron-3-Super-120B-A12B-BF16") else { return; };
    let cfg = parse_config(&dir).expect("config not found");
...
fn test_parse_devstral_123b() {
    let Some(dir) = find_snapshot("mistralai--Devstral-2-123B-Instruct-2512") else { return; };
    let cfg = parse_config(&dir).expect("config not found");
    assert_eq!(cfg.num_layers, 88);
    let attn = cfg.layers.iter().filter(|l| l.layer_type == LayerType::Attention).count();
...
fn test_parse_qwen3_coder_next() {
    let Some(dir) = find_snapshot("Qwen--Qwen3-Coder-Next") else { return; };
    let cfg = parse_config(&dir).expect("config not found");
    assert_eq!(cfg.num_layers, 48);
    let gdn = cfg.layers.iter().filter(|l| l.layer_type == LayerType::Gdn).count();
    let attn = cfg.layers.iter().filter(|l| l.layer_type == LayerType::Attention).count();
    ...
    println!("Qwen3-Coder-Next: {gdn} GDN + {attn} Attn, MoE {}/{}", cfg.num_active_experts, cfg.num_experts);
}
