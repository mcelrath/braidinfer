...
fn test_quantized_kv_vs_f32_paged() {
    ...
    let mut logits = model_f32
        .decode_step_paged(prompt_token, 0)
        .expect("f32 step 0");
    for i in 0..num_steps {
        let (tok, _) = argmax(&logits);
        f32_tokens.push(tok);
        logits = model_f32
            .decode_step_paged(tok as u32, (i + 1) as u32)
            .expect("f32 step");
    }
    ...
    let mut logits = model_q
        .decode_step_paged_quantized(prompt_token, 0)
        .expect("quant step 0");
    for i in 0..num_steps {
        let (tok, _) = argmax(&logits);
        q_tokens.push(tok);
        logits = model_q
            .decode_step_paged_quantized(tok as u32, (i + 1) as u32)
            .expect("quant step");
    }
