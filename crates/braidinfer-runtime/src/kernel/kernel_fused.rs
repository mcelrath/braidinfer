...
impl GqaAttentionKernel {
    ...
    pub fn forward(
        ...
        num_q_heads: u32,  // global nqh
        num_kv_heads: u32, // global nkh
        head_dim: u32,
        seq_len: u32,
        max_seq_len: u32,
        stream: &Stream,
        ...
        self.forward_ptr(
            ...
            seq_len,
            max_seq_len,
            num_q_heads, // single-GPU: local_nqh == global nqh
            0,           // single-GPU: q_head_start == 0
            stream,
        )
    }
    ...
    pub fn forward_ptr(
        ...
        k_cache: *const f32,
        v_cache: *const f32,
        num_q_heads: u32,  // global nqh (for gqa_group)
        num_kv_heads: u32, // global nkh
        head_dim: u32,
        seq_len: u32,
        max_seq_len: u32,
        local_num_q_heads: u32, // block count (q buffer size / head_dim)
        q_head_start: u32,      // gpu_i * local_nqh
        stream: &Stream,
    ) -> HipResult<()> {
        ...
        let mut qhs = q_head_start as i32;
        let mut args: [*mut c_void; 10] = [
            ...
            std::ptr::addr_of_mut!(qhs).cast(),
        ];
        let block_size = head_dim.min(256);
        let shared_bytes = block_size * 4;
        func.launch(
            (local_num_q_heads, 1, 1),
            (block_size, 1, 1),
            shared_bytes,
            stream,
            &mut args,
