...
impl MegakernelProgram {
    ...
    /// bd srg6.5 Phase 3c: one-shot paged-prefill compile (single-GPU).
    ///
    /// Compiles a megakernel program that runs N-token prefill with paged KV
    /// chunks (vs the legacy flat `legacy_kv_caches` of
    /// `compile_prefill_segment_with_module`). Each attention layer emits:
    ///   - batched mRoPE (N tokens)
    ///   - 2× OP_KV_WRITE_PAGED_BATCH (K, V)
    ///   - per-token OP_ATTN_PAGED with seq_len = start_pos + t + 1
    /// Non-attention layers (Gdn / Mamba2) emit verbatim from
    /// `compile_prefill_segment_with_module`. MoeFfn layers are skipped
    /// (handled by CPU prefill_mixed_chunk via moe_ffn_forward_prefill_batched).
    /// LfmConv panics — no current model ships it.
    ///
    /// Caller MUST:
    ///   - Pre-allocate `seq` + `allocator` with sufficient chunk capacity for N tokens.
    ///   - Provide host-mapped `page_table_buf` (≥ ceil(N/CHUNK_TOKENS) u64s) and
    ///     `position_table_buf` (≥ 3*N i32s) — this function writes chunk
    ///     base pointers + mRoPE positions before emitting instructions.
    ///   - Provide `prefill_bufs` sized for N tokens. Source/dest buffers
    ///     (q_attn, k_attn, v_attn, attn_out, etc.) are read by the megakernel
    ///     at execution time.
    ///
    /// KV_QUANT seal emission deferred to **braidinfer-gcga**.
    /// Token-ID match gate vs decode-loop reference deferred to **braidinfer-p3nz**.
